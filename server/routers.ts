...
export const appRouter = router({
  ...
  agents: router({
    ...
    createFromPrompt: protectedProcedure
      .input(z.object({ prompt: z.string().min(10).max(4000) }))
      .mutation(async ({ ctx, input }) => {
        const generated = await invokeLLM({
          ...
          response_format: {
            type: "json_schema",
            json_schema: { name: "agent_config", strict: true, schema: generatedAgentSchema },
          },
        });
        let config: any;
        try {
          config = JSON.parse(responseText(generated));
        } catch {
          config = {
            name: "New companion",
            tagline: "A thoughtful AI built from your idea.",
            icon: "✦",
            accent: "violet",
            systemPrompt: input.prompt,
            capabilities: ["Helpful conversation", "Clear explanations", "Creative thinking"],
          };
        }
        return createAgent({
          userId: ctx.user.id,
          ...
          capabilities: JSON.stringify(Array.isArray(config.capabilities) ? config.capabilities.slice(0, 5) : []),
        });
      }),
  ...
  chat: router({
    ...
    send: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [savedMessages, notes, memories] = await Promise.all([
          getMessages(ctx.user.id, chat.id),
          getKnowledge(ctx.user.id, agent.id),
          getMemories(ctx.user.id, agent.id),
        ]);
        const context = [
          notes.length ? `Knowledge notes:\n${notes.map((n) => `- ${n.title}: ${n.content}`).join("\n")}` : "",
          memories.length ? `Remember about the user:\n${memories.map((m) => `- ${m.content}`).join("\n")}` : "",
        ].filter(Boolean).join("\n\n");
        const userContent: any = input.imageDataUrl
          ? [{ type: "text", text: input.message }, { type: "image_url", image_url: { url: input.imageDataUrl, detail: "auto" } }]
          : input.message;
        const llmMessages: any[] = [
          { role: "system", content: `${agent.systemPrompt}\n\n${context}\n\nBe honest about limits. Keep answers warm, useful, and easy to scan.` },
  ...
  knowledge: router({
    list: protectedProcedure.input(z.object({ agentId: z.number().int().positive() })).query(({ ctx, input }) => getKnowledge(ctx.user.id, input.agentId)),
    add: protectedProcedure.input(z.object({ agentId: z.number().int().positive(), title: z.string().min(1).max(160), content: z.string().min(1).max(20000) })).mutation(({ ctx, input }) => createKnowledge({ userId: ctx.user.id, ...input })),
    remove: protectedProcedure.input(z.object({ id: z.number().int().positive() })).mutation(({ ctx, input }) => deleteKnowledge(ctx.user.id, input.id)),
  }),

  memory: router({
    list: protectedProcedure.input(z.object({ agentId: z.number().int().positive() })).query(({ ctx, input }) => getMemories(ctx.user.id, input.agentId)),
    add: protectedProcedure.input(z.object({ agentId: z.number().int().positive(), content: z.string().min(1).max(1000) })).mutation(({ ctx, input }) => createMemory({ userId: ctx.user.id, ...input })),
    remove: protectedProcedure.input(z.object({ id: z.number().int().positive() })).mutation(({ ctx, input }) => deleteMemory(ctx.user.id, input.id)),
  }),
});
